"""
This script implements a clustering algorithm for entities and edges in a knowledge graph.
It uses GPT-4 to dynamically group entities and edges into multiple clusters based on semantic similarity,
handling variations in tenses, plural forms, stem forms, and capitalization.
The process involves extracting multiple clusters simultaneously, validating them,
selecting representative items, and continuing until all items are processed.
This approach aims to reduce redundancy and improve the coherence of the knowledge graph
by grouping semantically equivalent or closely related entities and edges.
"""
...
EXTRACT_ENTITIES_CLUSTERS_PROMPT = """Find multiple clusters of related entities from this list.
...
VALIDATE_ENTITIES_CLUSTERS_PROMPT = """Verify whether these clusters belong together.
...
EXTRACT_EDGES_CLUSTERS_PROMPT = """Find mutiple cluster of closely related predicates from this list.
...
VALIDATE_EDGES_CLUSTERS_PROMPT = """Verify whether these clusters belong together.
A cluster should contain predicates that are the same in meaning, with different:
- tenses
- plural forms
- stem forms
...
class MultipleClustersResponse(BaseModel):
    clusters: List[List[str]]
    @model_validator(mode='after')
    def validate_entities_exist(self) -> 'MultipleClustersResponse':
        # This will be populated with the valid entities list before parsing
        if not hasattr(self, '_valid_entities'):
            return self
        
        # Filter out invalid entities from each cluster
        self.clusters = [[e for e in cluster if e in self._valid_entities] for cluster in self.clusters]
        # Remove empty clusters
        self.clusters = [cluster for cluster in self.clusters if cluster]
        # Double check that all remaining entities are valid
        assert all(all(e in self._valid_entities for e in cluster) for cluster in self.clusters)
        return self 
...
@handle_rate_limit
def extract_multiple_clusters(items: List[str], item_type: str) -> List[List[str]]:
    """Extract multiple clusters from the list of items"""
    items_text = "\n".join([f"- {item}" for item in items])
    items_set = set(items)
    
    logger.debug(f"Attempting to extract multiple {item_type} clusters from {len(items)} items")
    try:
        MultipleClustersResponse._valid_entities = items_set
        
        prompt = EXTRACT_ENTITIES_CLUSTERS_PROMPT if item_type == "entities" else EXTRACT_EDGES_CLUSTERS_PROMPT
        ...
        completion = client.beta.chat.completions.parse(
            ...
            response_format=MultipleClustersResponse,
            temperature=0
        )
        
        clusters = completion.choices[0].message.parsed.clusters
        if not clusters:
            logger.debug(f"No suitable {item_type} clusters found in batch")
        else:
            logger.info(f"Found {len(clusters)} {item_type} clusters")
            # Print the extracted clusters
            for i, cluster in enumerate(clusters):
                print(f"Cluster {i + 1}: {', '.join(cluster)}")
            
        del MultipleClustersResponse._valid_entities
        return clusters
...
@handle_rate_limit
def validate_clusters(clusters: List[List[str]], item_type: str) -> List[List[str]]:
    """Validate that cluster members are truly synonyms"""
    logger.debug(f"Validating {item_type} clusters with {len(clusters)} clusters")
    try:
        completion = client.beta.chat.completions.parse(
            model=MODEL,
            messages=[
                {"role": "system", "content": VALIDATE_ENTITIES_CLUSTERS_PROMPT if item_type == "entities" else VALIDATE_EDGES_CLUSTERS_PROMPT},
                {"role": "user", "content": f"{item_type.capitalize()} clusters: {', '.join(clusters)}"},
            ],
            response_format=MultipleClustersResponse,
            temperature=0
        )
        validated = completion.choices[0].message.parsed.clusters if completion.choices[0].message.parsed.clusters else []
        logger.debug(f"Validation result: {len(validated)}/{len(clusters)} {item_type} clusters validated")
        return validated
    except Exception as e:
...
def choose_representative(cluster: List[str]) -> str:
    """Choose the best item name to represent the cluster"""
    ...
    for attempt in range(max_retries):
        try:
            completion = client.beta.chat.completions.parse(
                model=MODEL,
                messages=[
                    {"role": "system", "content": """Select the best item name in this cluster to represent the cluster.
    ...
    try:
        ...
        for i, index in enumerate(cluster_indices):
            if index is not None:
                ...
                validated_clusters = validate_clusters([new_cluster], item_type)
                
                if not validated_clusters or len(validated_clusters[0]) != len(clusters[index].items) + 1:
...
def cluster_items(items: Set[str], item_type: str = "entities") -> List[Cluster]:
    ...
    while remaining_items:
        ...
        suggested_clusters = extract_multiple_clusters(remaining_items, item_type)
        validated_clusters = validate_clusters(suggested_clusters, item_type)
        
        if validated_clusters:
            no_progress_count = 0
            for validated_cluster in validated_clusters:
            ...
            no_progress_count += 1
            logger.info(f"No valid clusters found. Attempts without progress: {no_progress_count}")
